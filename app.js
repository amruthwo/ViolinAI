...
(function () {
  ...
  let mode = "preview";
  let bpm = 120;
  let keySig = null; // {sf, mi} if found
  ...
  let previewPausedSongTime = 0; // song-time seconds (scaled)
  ...
  async function loadMidi(arrayBuffer){
    ...
    // key signature (ToneJS sometimes provides sf/mi, sometimes not)
    keySig = null;
    ...
    // collapse chords to highest pitch at same time
    const collapsed=[];
    const EPS=0.03;
    for(const n of raw){
  ...
  function parseMusicXML(xmlText){
    ...
    // tempo (best effort): first <sound tempo="..."> or <per-minute>
    bpm = 120;
    ...
    // key signature: <fifths> and optional <mode>
    keySig = null;
    ...
    // Choose part: pick first <part>, but prefer id containing "violin" if present
    ...
    for (const meas of measures) {
      // divisions can change per measure
      ...
      for (const n of notesEl) {
        ...
        const quarterLen = divisions > 0 ? (durDiv / divisions) : 0; // in quarter notes
        ...
        if (!isRest) {
          ...
          if (step && octave) {
            ...
            if (midi != null) {
              out.push({ t: startSec, dur: durSec, midi });
            }
          }
        }
        ...
        if (!chord) {
          ...
          // chord note shares start time with previous note, does not advance time
          lastStartSec = startSec;
        }
      }
    }

    // Collapse simultaneous notes (chords) to highest pitch for learning lane
